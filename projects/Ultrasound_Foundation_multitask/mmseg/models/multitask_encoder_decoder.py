...
class MultitaskEncoderDecoder(BaseSegmentor):
    ...
    def predict(self,
        ...
        seg_logits, cls_logits = self.inference(inputs, batch_img_metas)

        return self.postprocess_result(seg_logits, cls_logits, data_samples)
    ...
    def slide_inference(self, inputs: Tensor,
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                # with shape [N, C, H, W]
                crop_seg_logit, crop_cls_logits = self.encode_decode(crop_img, batch_img_metas)
                preds += F.pad(crop_seg_logit,
                               (int(x1), int(preds.shape[3] - x2), int(y1),
                                int(preds.shape[2] - y2)))
                preds_cls += crop_cls_logits

                count_mat[:, :, y1:y2, x1:x2] += 1
        assert (count_mat == 0).sum() == 0
        seg_logits = preds / count_mat

        cls_logits = preds_cls / count_mat
    ...
    def whole_inference(self, inputs: Tensor,
        ...
        seg_logits, cls_logits = self.encode_decode(inputs, batch_img_metas)
    ...
    def inference(self, inputs: Tensor, batch_img_metas: List[dict]) -> Tensor:
        ...
        if self.test_cfg.mode == 'slide':
            seg_logit, cls_logit = self.slide_inference(inputs, batch_img_metas)
        else:
            seg_logit, cls_logit = self.whole_inference(inputs, batch_img_metas)
    ...
    def aug_test(self, inputs, batch_img_metas, rescale=True):
        ...
        seg_logit, cls_logit = self.inference(inputs[0], batch_img_metas[0], rescale)
        for i in range(1, len(inputs)):
            cur_seg_logit, cur_cls_logit = self.inference(inputs[i], batch_img_metas[i],
                                                           rescale)
            seg_logit += cur_seg_logit
            cls_logit += cur_cls_logit
        seg_logit /= len(inputs)
        cls_logit /= len(inputs)
        seg_pred = seg_logit.argmax(dim=1)
        # unravel batch dim
        seg_pred = list(seg_pred)
        cls_pred = list(cls_logit)
